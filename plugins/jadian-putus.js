...
let ps = groupMetadata.participants.map(v => v.id)
  ...
  try {
    ...
    if(!users) return conn.reply(m.chat, `*_❗Doi Atau Nomor Tidak Ditemukan, Mungkin Sudah Keluar Atau Bukan Anggota Grup Ini.*_`, m)
    if(user === m.sender) return conn.reply(m.chat, `_*❗Tidak Bisa Berpacaran Dengan Diri Sendiri.*_`, m)
    if(user === conn.user.jid) return conn.reply(m.chat, `_*❗Maaf Kak, Tapi Saya Sepenuhnya Milik Kak yuki.*_`, m)
    ...
    if(global.db.data.users[m.sender].pasangan != "" && global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender && global.db.data.users[m.sender].pasangan != user){
      ...
      if (pacar){
        if (m.sender == pacar && global.db.data.users[m.sender].pasangan == user) return conn.reply(m.chat, `❣️Kamu Sudah Berpacaran Dengan ${spac.split('@')[0]}`, m , { contextInfo: { mentionedJid: [spac]}})
      ...
      global.db.data.users[m.sender].pasangan = user
      conn.reply(m.chat, `❣️Selamat Kamu Resmi Berpacaran Dengan @${user.split('@')[0]}\n\nSemoga Langgeng Dan Bahagia Selalu 🥳🥳🥳`, m , { contextInfo: { mentionedJid: [user]}})
    }else {
      global.db.data.users[m.sender].pasangan = user
      conn.reply(m.chat, `${await ktnmbk.getRandom()}\n\nKamu Baru Saja Mengajak @${user.split('@')[0]} Berpacaran\n\nSilahkan Menunggu Jawaban Darinya!\n\nKetik *${usedPrefix}terima @user* Untuk Menerima\n*${usedPrefix}tolak @user Untuk Menolak*`, m , { contextInfo: { mentionedJid: [user]}})
    }
